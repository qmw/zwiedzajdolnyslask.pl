...
vercel.json         build config, the smart /app redirect, /api/zgloszenie rewrite
server/             PHP bug-report handler (runs on LH.pl, not Vercel)
```
...
| everything else                           | `public/app/index.html` — a tiny JS check catches tablets that send a *desktop* UA (Chrome on Android tablets, iPad in desktop mode) via touch detection and sends them to the right store; real desktops go to the landing page (`/`, `/en` or `/de` by browser language, `#pobierz` section) |
...
- Requires JavaScript (fetch + fill-time check).
